...
class TextInput:
    ...
    def update(self, events, cap, list_of_chars, screen_max_x, screen_max_y, split_mode_on):
        for event in events:
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pl.K_BACKSPACE: # FIXME: Delete at beginning of line?
                    self.input_string = self.input_string[:max(self.cursor_position - 1, 1)] + \
                    ...
                    for i in list_of_chars:
                        if not split_mode_on:
                            if abs(i.textrect.centerx-screen_max_x//2)<33:
                                if i.r in cap:
                                    cap.remove(i.r)
                                else:
                                    cap.append(i.r)
                            ...
                            if (abs(i.textrect.centerx-screen_max_x//4)<33) or (abs(i.textrect.centerx-3*screen_max_x//4)<33):
                                if i.r in cap:
                                    cap.remove(i.r)
                                else:
                                    cap.append(i.r)
        ...
        for key in self.keyrepeat_counters :
            ...
            if self.keyrepeat_counters[key][0] >= self.keyrepeat_intial_interval_ms:
                ...
                #self.keyrepeat_counters[key][0]=0
            else:
                pygame.event.post(pygame.event.Event(pl.KEYUP, key=key, unicode=self.keyrepeat_counters[key][1]))
